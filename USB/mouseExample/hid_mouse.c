...
{
	//	Initialize GPIO's as inputs
	for(int i = 0; i < 5; i++)
		DigitalIn(GPIO[i][0], GPIO[i][1]);
}*/

/*
static uint8_t button_status(void)
{
	uint8_t ix, ret = 0;

	for (ix = 0; ix < 5; ix++) {
		if ((DigitalRead(GPIO[ix][0], GPIO[ix][1])) == false) {
			ret |= stateButton[ix];
		}
	}
...
/* Mouse adjusting speed */
void Mouse_adjust(int delay_ms)
{
	/* check device is configured before sending report. */
	if ( USB_IsConfigured(g_mouse.hUsb)) {
		if (g_mouse.tx_busy == 0) {
			/* update report based on board state */
			delayMs(delay_ms);	//	Delay mouse update
			Mouse_UpdateReport();
			/* send report data */
			g_mouse.tx_busy = 1;
			USBD_API->hw->WriteEP(g_mouse.hUsb, HID_EP_IN, &g_mouse.report[0], MOUSE_REPORT_SIZE);
